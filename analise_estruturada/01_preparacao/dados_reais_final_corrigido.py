...
# 9. QUALIDADE - MÉDIAS
print('9. QUALIDADE - MEDIAS (escala 1-5):')
mapa_satisfacao = {
...
print('10. PERCEPCAO DE RECOMPENSAS - MEDIAS (escala 1-5):')
mapa_concordancia = {
...
print('11. INTENCAO COMPORTAMENTAL - MEDIAS (escala 1-5):')
...
print('=== RESUMO GERAL ===')
print(f'Media geral qualidade: {qualidade_num.iloc[:, 1:].mean().mean():.2f}')
print(f'Media geral percepcao: {percepcao_num.iloc[:, 1:].mean().mean():.2f}')
print(f'Media geral intencao: {intencao_num.iloc[:, 1:].mean().mean():.2f}')

# 12. CORRELAÇÕES PARA SEM
print('\n=== CORRELACOES PARA MODELO SEM ===')
...
print(f'Correlacao Qualidade <-> Intencao: {corr_matrix.loc["Qualidade", "Intencao_Comportamental"]:.3f}')
print(f'Correlacao Percepcao <-> Intencao: {corr_matrix.loc["Percepcao_Recompensas", "Intencao_Comportamental"]:.3f}')
print(f'Correlacao Qualidade <-> Percepcao: {corr_matrix.loc["Qualidade", "Percepcao_Recompensas"]:.3f}')
...
print(f'\nR² Percepcao -> Intencao: {r2_perc:.3f}')
print(f'R² Qualidade -> Intencao: {r2_qual:.3f}')
print(f'R² Modelo Completo: {r2_both:.3f}')
print(f'Coeficiente Percepcao: {model_both.coef_[1]:.3f}')
print(f'Coeficiente Qualidade: {model_both.coef_[0]:.3f}')
print(f'Casos validos para SEM: {len(construtos_clean)}')
...
print(f'Ensino medio ou superior: {ensino_medio_superior} ({ensino_medio_superior/total*100:.1f}%)')

# Negros (pretos + pardos)
negros = raca[raca.index.str.contains('Negra')].sum()
print(f'Populacao negra: {negros} ({negros/total*100:.1f}%)')
...
print(f'Usuarios de TP: {usuarios_tp} ({usuarios_tp/len(utilizacao)*100:.1f}%)')

print('\n=== DADOS CORRETOS CONFIRMADOS ===')
print('Todos os valores acima sao REAIS dos dados originais!')

# Salvar resultados principais
output_dir = os.path.join(os.path.dirname(__file__), '..', 'outputs', 'dados_processados')
os.makedirs(output_dir, exist_ok=True)

# Salvar construtos principais
construtos_clean.to_csv(os.path.join(output_dir, 'construtos_principais.csv'), index=False)
print(f'\nConstrutos salvos em: {output_dir}/construtos_principais.csv')
